...
/**
 * SEO config for Property Listing pages by city.
 * e.g. city=delhi → "/properties-in-delhi" page SEO
 */
@Entity('property_city_seo')
@Index(['citySlug'], { unique: true })
export class PropertyCitySeo {
  @PrimaryGeneratedColumn('uuid')
  id: string;

  @Column({ name: 'city_slug', length: 150 })
  citySlug: string; // 'delhi', 'noida'

  @Column({ name: 'city_name', length: 150 })
  cityName: string; // 'Delhi', 'Noida'

  /** URL slug: e.g. properties-in-delhi */
