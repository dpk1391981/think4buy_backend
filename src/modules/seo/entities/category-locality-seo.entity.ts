...
/**
 * SEO config for Category + Locality (city+locality) combinations.
 * Highest priority in resolver.
 * e.g. category=buy + city=gurgaon + locality=sector-56
 */
@Entity('category_locality_seo')
@Index(['categorySlug', 'citySlug', 'localitySlug'], { unique: true })
export class CategoryLocalitySeo {
  @PrimaryGeneratedColumn('uuid')
  id: string;

  @Column({ name: 'category_slug', length: 100 })
  categorySlug: string; // 'buy', 'rent', etc.
  ...
  /** URL slug: e.g. flats-for-sale-in-gurgaon-sector-56 */
